...
def freq_items(item_df):
    return item_df['aisle_id'].value_counts().rename('freq')
...
def item_sets(item_df):
    m = item_df[['order_id','aisle_id']].as_matrix()
...
def association_rules(item_df, min_support):
    '''
    Takes a dataframe with columns ['order_id','aisle_id']
    ...
    qualifying_orders = more_than2[more_than2['aisle_id'].isin(qualifying_items)]['order_id'].unique()
...
orders_products_df = orders_products_df[['order_id','aisle_id']]

rules = association_rules(orders_products_df, float(min_support_input))

aisles_name = pd.read_csv('../data/aisles.csv')

rules_final = merge_item_name(rules, aisles_name).sort_values('lift', ascending=False)
